...
class CollapsedSampler:
	def __init__(self, training_data, 
		...
		nav_topic_mean_prior_covariance_kappa, nav_topic_covariance_prior_dof,
		...
		self.nav_topic_mean_prior_covariance_kappa = nav_topic_mean_prior_covariance_kappa
	...
	def calculate_nav_intermediate_values(self):
		...
		self.nav_topic_stats = defaultdict(lambda: [set(), 0, np.zeros(self.training_data.nav_embeddings.shape[1])])
		...
		for article_id in range(len(self.training_data.article_navs)):
			for article_nav_id, nav_id in enumerate(self.training_data.article_navs[article_id]):
				...
				self.nav_topic_stats[nav_topic_assignment][0].add((article_nav_id, nav_id))
	...
	def get_nav_topic_mean_cov_dof(self, nav_topic_id):
		topic_nav_ids, topic_nav_count, topic_nav_sum = self.nav_topic_stats[nav_topic_id]

		topic_nav_ids = list(map(lambda t: t[1], topic_nav_ids))
		topic_navs = self.training_data.nav_embeddings[topic_nav_ids]
		topic_nav_sum = sum(topic_navs)
		topic_nav_mean = (1 / topic_nav_count) * topic_nav_sum

		updated_kappa = self.nav_topic_mean_prior_covariance_kappa + topic_nav_count
		updated_dof  = self.nav_topic_covariance_prior_dof + topic_nav_count
		updated_mean = topic_nav_sum / updated_kappa

		topic_nav_devs = topic_navs - topic_nav_mean
		updated_scale = self.nav_topic_covariance_prior_scale + \
			np.einsum('ki,kj->ij', topic_nav_devs, topic_nav_devs) + \
			(((self.nav_topic_mean_prior_covariance_kappa * topic_nav_count)/updated_kappa) *
				topic_nav_mean @ topic_nav_mean.transpose())
		updated_cov = updated_scale / (updated_dof - 300 + 1)
		return updated_mean, updated_cov, updated_dof
	...
	def sample_nav_article_nav_assignment(self, article_id, article_nav_id, nav_id):
		...
		for nav_topic_id in range(self.num_nav_topics):
			updated_mean, updated_cov, updated_dof = \
				self.get_nav_topic_mean_cov_dof(nav_topic_id)
			assignment_proportions[nav_topic_id] += \
				calculate_multivariate_t_logpdf(
					nav_embedding, updated_mean, updated_cov, updated_dof
				)
		...
		if old_assignment != updated_assignment:
			...
			self.nav_topic_stats[old_assignment][0].remove((article_nav_id, nav_id))
			self.nav_topic_stats[old_assignment][1] -= 1
			self.nav_topic_stats[old_assignment][2] -= nav_embedding

			self.nav_topic_stats[updated_assignment][0].add((article_nav_id, nav_id))
			self.nav_topic_stats[updated_assignment][1] -= 1
			self.nav_topic_stats[updated_assignment][2] -= nav_embedding
	...
	def run(self, num_iterations):
		...
		for epoch in range(num_iterations):
			...
			means, covs = [], []
			for nav_topic_id in range(self.num_nav_topics):
				mean, cov, _ = self.get_nav_topic_mean_cov_dof(nav_topic_id)
				means.append(mean)
